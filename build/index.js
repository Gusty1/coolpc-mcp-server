...
class CoolPCMCPServer {
    ...
    searchCPU(args) {
        ...
        const results = [];
        // Find CPU category
        const cpuCategory = this.productData.find(cat => cat.category_name.includes('CPU') || cat.category_name.includes('處理器'));
        if (!cpuCategory) {
            return {
                content: [
                    {
    ...
    searchGPU(args) {
        ...
        const results = [];
        // Find GPU category
        const gpuCategory = this.productData.find(cat => cat.category_name.includes('顯示卡') || cat.category_name.includes('VGA'));
        if (!gpuCategory) {
            return {
                content: [
                    {
    ...
    searchRAM(args) {
        ...
        const results = [];
        // Find RAM category
        const ramCategory = this.productData.find(cat => cat.category_name.includes('記憶體') || cat.category_name.includes('RAM'));
        if (!ramCategory) {
            return {
                content: [
                    {
    ...
    searchSSD(args) {
        ...
        const results = [];
        // Find SSD category
        const ssdCategory = this.productData.find(cat => cat.category_name.includes('SSD') || cat.category_name.includes('固態硬碟'));
        if (!ssdCategory) {
            return {
                content: [
                    {
    ...
    searchMotherboard(args) {
        const { socket, chipset, form_factor, sort_by, limit = 10 } = args;
        const results = [];
        // Find motherboard category
        const mbCategory = this.productData.find(cat => cat.category_name.includes('主機板') || cat.category_name.includes('MB'));
        if (!mbCategory) {
            return {
                content: [
                    {
    ...
    getCategoryProducts(args) {
        ...
        if (subcategory_name) {
            ...
            const subcategory = category.subcategories.find(sub => sub.name.toLowerCase() === subcategory_name.toLowerCase());
            if (!subcategory) {
                return {
                    content: [
                        {
